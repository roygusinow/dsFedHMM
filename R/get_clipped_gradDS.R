...
get_clipped_gradDS <- function(
  ...
  # grab dp params from datashield system installation
  thr <- dsBase::listDisclosureSettingsDS()
  clipping_threshold <- thr$clipping_threshold
  noise_multiplier <- thr$noise_multiplier

  # only difference for clipped
  next_grad <- LTA$get_clipped_grad(
    current_parameters,
    LTS_output,
    clipping_threshold, # clipping norm
    0.0 # lambda
  )

  # simple dp sgd mechanism for now... need actual batching
  noise_std <- noise_multiplier * clipping_threshold
  noise <- rnorm(length(next_grad), mean = 0, sd = noise_std)

  # 7. Add noise to the aggregated gradient
  dp_aggregated_grad <- next_grad + noise


  return(list(
    dp_aggregated_grad = dp_aggregated_grad,
